...
# ---
# 
# * Estudiante: **Victor Felipe Rojas Vargas**
# * Carné: **B87001**
# * Grupo: **1**
# 
# ---
...
#Para esta primera parte se utilizara parte del código usado en la función modulador y demodulador, pero modificandolas para que realice modulación 16-QAM en vez de BPSK.
#La modulación 16-QAM (*Quadrature Amplitude Modulation*) tiene cuatro símbolos posibles (`00`, `01`, `10`, `11`), con cuatro bits $b_1 b_2 b_3 b_4$ por símbolo.
...
def modulador16QAM(bits, fc, mpp):
    ...
    for i in range(0, len(bits), 1):
        for j in range(1,len(bits), 1):
            for k in range(2,len(bits), 1):
                for l in range(3,len(bits), 1):
                    bit1 = bits[i]
                    bit2 = bits[j]
                    bit3 = bits[k]
                    bit4 = bits[l]
                    ...
                    if bit3 == 0 and bit4 == 0:
                        senal_Tx1[j*mpp : (j+1)*mpp] = (portadora2 * 3)
                    elif bit3 == 0 and bit4 == 1:
                        senal_Tx1[j*mpp : (j+1)*mpp] = (portadora2 * 1)
...
#Una vez creado el modulador, se procede a reutilizar el código de la parte de arriba usado para mostrar de ejemplo como usar las funciones previamente creadas:
import numpy as np
import matplotlib.pyplot as plt
import time
...
senal_Tx, Pm, portadora = modulador16QAM(bits_Tx, fc, mpp)
...
# 9. Mostrar imagen transmitida
ax = Fig.add_subplot(1, 2, 1)
imgplot = plt.imshow(imagen_Tx)
ax.set_title('Transmitido')

# 10. Mostrar imagen recuperada
ax = Fig.add_subplot(1, 2, 2)
imgplot = plt.imshow(imagen_Rx)
ax.set_title('Recuperado')
Fig.tight_layout()
...
#Tomando como base parte del código del laboratorio 4, se puede proceder a realizar este punto:

t = np.linspace(0, 10, 100)
A = [-1,1]
s_t = np.empty((4, len(t)))

#Se obtienen los valores de cada función
for j in A: 
...
plt.title("Proceso aletorio S(t)")
plt.xlabel("$t$")
plt.ylabel("$s(t)$")
plt.legend()
...
#Para la parte final, se tiene que calcular la Transformada de Fourier para obtener muestras, de ahi el porque se importa fft
...
#Finalmente se le da formato al ploteo
plt.plot(f, 2.0/Nsample * np.power(np.abs(senal_f[0:Nsample//2]), 2))
plt.xlim(0, 20000)
plt.grid()
plt.show()
